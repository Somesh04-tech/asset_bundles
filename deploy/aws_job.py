...
parser.add_argument('--env', default = "qa", help="environment to deploy to")
...
fil.write(f"""resources:
  jobs:
    Fl-Communication-Mail:
      name: Fl-Communication-Mail""")
if (notify)|(env=="PROD"):
  fil.write(f"""
      email_notifications:
        on_start:
        - ANTDLMOC@zebra.com
        - AntuitDataOpsInternal@zebra.com
        ...
        - AntuitDataOpsInternal@zebra.com
        - ranjith.kl@zebra.com
        - antuit-moc@reflexisinc.pagerduty.com
        on_Duration warning:
        - antuit-moc@reflexisinc.pagerduty.com
        - antuit-dataops-emergency@reflexisinc.pagerduty.com""")
fil.write(f"""
      tags:
        job_name: Fl-Communication-Mail
        workspace: {env}""")
if env=="PROD":
  fil.write(f"""
      schedule:
        quartz_cron_expression: 0 0 17 ? * FRI
        timezone_id: UTC
